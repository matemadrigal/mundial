...
import { apiReady, fetchFixtures, fetchMatchStats, fetchEventDetail, simplifyStatus } from './apifootball';
...
export async function runSync(mode = 'light') {
  ...
  try {
    ...
    // 3) Enriquecer con letra REAL de grupo (FIFA). El listado de season
    //    de TheSportsDB no la trae; lookupevent sí. Solo enriquecemos los
    //    matches cuyo stage todavía sea genérico ("Group Stage - …"). En
    //    cuanto un partido tiene "Group A", "Group B", etc., nunca se
    //    refetchea. Cap por sync para no quemar la API.
    let enriched = 0;
    let enrichErrors = 0;
    try {
      const { data: needGroup } = await sb
        .from('matches')
        .select('id,stage')
        .or(`stage.is.null,stage.ilike.%Group Stage%`)
        .limit(30);
      for (const m of needGroup || []) {
        try {
          const d = await fetchEventDetail(m.id);
          if (!d || !d.strGroup) continue;
          const letter = String(d.strGroup).trim().match(/^[A-L]$/i)?.[0]?.toUpperCase();
          if (!letter) continue;
          const jornadaMatch = (m.stage || '').match(/jornada\s*(\d+)/i);
          const stageNew = jornadaMatch
            ? `Group ${letter} - Jornada ${jornadaMatch[1]}`
            : `Group ${letter}`;
          const update = { stage: stageNew };
          if (d.strCity) update.city = d.strCity;
          await sb.from('matches').update(update).eq('id', m.id);
          enriched++;
        } catch {
          enrichErrors++;
          // lookupevent falló para este id: lo reintentamos en el siguiente sync
        }
      }
    } catch {
      // El select inicial falló: no podemos enriquecer pero el sync sigue siendo válido.
      enrichErrors++;
    }

    await setSetting('last_sync', { at: new Date().toISOString(), mode, updated, statsFetched, enriched, enrichErrors });
    return { ok: true, mode, updated, statsFetched, enriched, enrichErrors };
  } finally {
