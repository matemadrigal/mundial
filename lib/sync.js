...
export async function runSync(mode = 'light') {
  ...
  try {
    ...
    //    matches cuyo stage aún no contiene "Group [A-L]", así nunca
    //    refetcheamos lo ya conocido. Cap por sync para no quemar la API.
    let enriched = 0;
    const { data: needGroup } = await sb
      .from('matches')
      .select('id,stage')
      .or(`stage.is.null,stage.not.ilike.%Group %`)
      .limit(30);
    for (const m of needGroup || []) {
      try {
        ...
        const stageNew = jornadaMatch
          ? `Group ${d.strGroup} - Jornada ${jornadaMatch[1]}`
          : `Group ${d.strGroup}`;
        await sb.from('matches').update({
          stage: stageNew,
          ...(d.strCity ? { city: d.strCity } : {})
        }).eq('id', m.id);
        enriched++;
      } catch {
    ...
    await setSetting('last_sync', { at: new Date().toISOString(), mode, updated, statsFetched, enriched });
    return { ok: true, mode, updated, statsFetched, enriched };
  } finally {
